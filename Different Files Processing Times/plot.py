...
categories = ['BOM\n(CSV)\nComponent AAS\nSM-Capability\nSM-MechanicalBreakdown', 'Datasheets\n(PDF)\nComponent AAS', 'CAD\n(STP/STEP)\nSM-Geometry',
              'PLC\n(PLCOpenXML)\nSM-PLC\nSM-OperationalData', 'SysML\n(UML)\nSM-Behavior',
              'MATLAB/Simulink\n (SLX)\nSM-Simulation', 'ECAD\n(E3S)\nSM-ECAD',  
              'Technical Reports\n(PDF)\nBOM']
automatic = [1.725, 7.47291176470588, 14.77388, 53.273, 798.092, 33.999, 2.864, 0]
manual_actual = [0, 0, 0, 0, 0, 7200, 12000, 36000]
...
for i, (auto, wh) in enumerate(zip(automatic, manual_display)):
    ...
    if wh > 0:
        ...
        ax.text(i, y_pos, '', ha='center', va='center',
                color='black', fontsize=200, fontweight='bold')
