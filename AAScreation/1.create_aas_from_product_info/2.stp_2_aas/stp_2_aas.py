...
This script processes STEP (.stp) files to extract 3D model data and converts 
it into an Asset Administration Shell (AAS) format. 

- Place the STEP files in the designated directory.
- Run the script to generate corresponding AAS files.
...
class aasFromSTP:
    ...
    def replace_str(self, s: str) -> str:
        ...
        if not s:
            s = "ID_unknown"
        # guarantee that the first character is a letter
        if not s[0].isalpha():
            s = "ID_" + s
        return s
...
class STPParser:
    ...
    def extract_identification(self):
        # fix: improved regex parsing for robustness
        with open(self.filename, 'r', encoding='latin-1', errors='ignore') as f:
            content = f.read()
        ...
        if m:
            desc_list = re.findall(r"'([^']*)'", m.group(1))
            self.identification['File_description']['Description'] = "; ".join(
                d.strip() for d in desc_list if d.strip()
            )
            self.identification['File_description']['Implementation_level'] = m.group(2).strip()

        # file_name
        m = re.search(
            r"FILE_NAME\s*\(\s*'([^']*)'\s*,\s*'([^']*)'\s*,\s*\((.*?)\)\s*,\s*\((.*?)\)\s*,\s*'([^']*)'\s*,\s*'([^']*)'\s*,\s*'([^']*)'\s*\)\s*;",
            content, re.IGNORECASE | re.DOTALL
        )
        if m:
            name = m.group(1).strip()
            time_stamp = m.group(2).strip()
            authors = re.findall(r"'([^']*)'", m.group(3))
            orgs = re.findall(r"'([^']*)'", m.group(4))
            preproc = m.group(5).strip()
            origin = m.group(6).strip()
            auth = m.group(7).strip()

            self.identification['File_name'].update({
                'name': name,
                'time_stamp': time_stamp,
                'author': "; ".join(a.strip() for a in authors) or "",
                'organization': "; ".join(o.strip() for o in orgs) or "",
                'preprocessor_version': preproc,
                'originating_system': origin,
                'authorisation': auth
            })
        else:
            # fallback for simpler FILE_NAME formats
            m2 = re.search(r"FILE_NAME\s*\((.*?)\)\s*;", content, re.IGNORECASE | re.DOTALL)
            if m2:
                vals = re.findall(r"'([^']*)'", m2.group(1))
                keys = ['name','time_stamp','author','organization','preprocessor_version','originating_system','authorisation']
                for i, k in enumerate(keys):
                    self.identification['File_name'][k] = (vals[i].strip() if i < len(vals) else "")
        ...
        if m:
            schemas = re.findall(r"'([^']*)'", m.group(1))
            self.identification['File_schema'] = (schemas[0].strip() if schemas else "")
...
if __name__ == '__main__':
    ...
    product_files_dir = os.path.join(parent_dir, "product_files")
    
    balluff_dir = os.path.join(product_files_dir, "Balluff")
    bernstein_dir = os.path.join(product_files_dir, "Bernstein")
    contelec_dir = os.path.join(product_files_dir, "Contelec")
    hbm_dir = os.path.join(product_files_dir, "HBM")
    ott_dir = os.path.join(product_files_dir, "Ott")
    festo_dir = os.path.join(product_files_dir, "Festo")
    phonix_dir = os.path.join(product_files_dir, "Phonix Contact")
    siemens_dir = os.path.join(product_files_dir, "Siemens")

    stp_files = glob.glob(os.path.join(balluff_dir, "*.stp")) + \
                glob.glob(os.path.join(bernstein_dir, "*.stp")) + \
                glob.glob(os.path.join(contelec_dir, "*.stp")) + \
                glob.glob(os.path.join(phonix_dir, "*.stp")) + \
                glob.glob(os.path.join(hbm_dir, "*.stp")) + \
                glob.glob(os.path.join(ott_dir, "*.stp")) + \
                glob.glob(os.path.join(siemens_dir, "*.stp")) + \
                glob.glob(os.path.join(festo_dir, "*.stp"))
